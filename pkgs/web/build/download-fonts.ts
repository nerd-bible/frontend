...
const cssOutPath = "./src/css/fonts.css";
...
for (const [familyName, faces] of Object.entries(fontFamilies)) {
	for (const [fontName, url] of Object.entries(faces)) {
		const endPath = join("fonts", familyName, `${fontName}.ttf`);
		const path = join("public", endPath);
...
@font-face {
	...
	font-display: swap;
	src: local('${face.name}'), url('/${face.path}') format('woff2');
