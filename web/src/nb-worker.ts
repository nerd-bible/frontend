...
function parseConlluDoc(
	...
	const wordCols = {
		...
		sentId: [] as number[],
		id: [] as number[],
		isElision: [] as (boolean | undefined)[],
		morphemeOf: [] as (number | undefined)[],
	...
	for (let i = 0; i < parsed.output.length; i++) {
		...
		let morphemesUntil = "";
		let morphemeOf: number | undefined;
		let isEllision: boolean | undefined;
		...
		for (let j = 0; j < sent.words.length; j++) {
			...
			wordCols.isElision.push(isEllision);
			wordCols.morphemeOf.push(morphemeOf);
			...
			if (w.id.includes("-")) morphemeOf = wordId;
		}
		for (let j = 0; j < sent.words.length; j++) {
			const w = sent.words[j];
